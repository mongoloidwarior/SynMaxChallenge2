...
for i in range (0, voyage.shape[0]):
...
#        stopped
    ...
    elif voyage.loc[voyage.index[i],'vessel'] != current_vessel:
        moving = voyage.loc[voyage.index[i],'speed']>0
        current_vessel = voyage.loc[voyage.index[i],'vessel']
        if moving == 0:
            last_port = voyage.loc[voyage.index[i],'port']            
    
    if addrow==1:
        rowtoadd = pd.DataFrame([[current_vessel,begin_date,last_port,end_date,curr_port]])
        voyages = pd.concat([voyages,rowtoadd])
        addrow=0

voyages

voyages.columns = ['vessel','begin_date','begin_port_id','end_date','end_port_id']
...
cat_cols = list(X_selected.select_dtypes(['float64','int64','int32']).columns.values)
...
for col in col_names:
    X_categorical[col] = X_categorical[col].astype('category',copy=False)

#X_train_selected = X_train_numerical.merge(X_categorical, on='vessel', how='left')
X_train_selected = X_categorical.copy()
